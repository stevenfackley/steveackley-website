...
async function main() {
  ...
  if (passwordHash) {
    // Docker compose requires escaping $ with $$, but we need single $ for bcrypt
  ...
  if (!passwordHash || passwordHash.startsWith("$2b$12$replace")) {
    ...
    console.log("\nGenerated hash. Add this to your .env.local file (single $):");
    console.log(`ADMIN_PASSWORD_HASH="${passwordHash}"`);
    console.log("\nFor .env (Docker Compose), escape $ as $$ like this:");
